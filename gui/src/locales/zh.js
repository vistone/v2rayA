export default {
  ...
  setting: {
    ...
    options: {
      ...
      updateGfwlistAtIntervals: "每隔一段时间更新GFWList（单位：小时）",
	      ...
	      advanced: "自定义高级设置",
	      leastPing: "最小时延优先",
	      randomParallel: "随机并行（推荐）：连接分发到所有节点，叠加出口带宽",
	      roundRobinParallel: "轮询并行：连接轮流分发到所有节点，叠加出口带宽",
  ...
  pool: {
    ...
    nameRequired: "请输入池名称",
    outbound: "出站名（可选）",
    outboundHint: "留空则使用池名称",
    members: "成员（勾选并填写每台 VPS 的 Agent 地址）",
    ...
    inPool: "在池",
    excluded: "已摘除",
  	  trafficGatePct: "流量门槛 %（达到即摘除）",
  	  pollInterval: "轮询间隔",
  	  failOpen: "全部成员被摘除时放回最优成员（fail-open）",
  	  strategy: "出站策略",
	  strategyRandom: "随机并行：多连接分发到所有节点，叠加带宽（短连接场景可能变慢）",
	  strategyRoundRobin: "轮询并行：多连接轮流分发到所有节点，叠加带宽（短连接场景可能变慢）",
	  strategyLeastPing: "最低延迟：长连接复用，单连接速度最快（默认）",
  	  agentURL: "Agent 地址，如 http://1.2.3.4:19528",
  	  agentToken: "Agent Token",
    controlOk: "控制指令已发送：{action}",
  },
