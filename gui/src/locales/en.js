export default {
  ...
  setting: {
    ...
    options: {
      ...
      updateGfwlistAtIntervals: "Update GFWList Regularly (Unit: hour)",
	      ...
	      randomParallel: "Random parallel (recommended): spread connections across all nodes, add bandwidth",
	      roundRobinParallel: "Round-robin parallel: connections alternate across all nodes, add bandwidth",
  ...
  pool: {
    ...
    outboundHint: "Defaults to pool name",
    members: "Members (check and fill each VPS agent URL)",
    ...
    inPool: "In pool",
    excluded: "Excluded",
    trafficGatePct: "Traffic gate % (exclude at or above)",
    pollInterval: "Poll interval",
  	  failOpen: "Re-add best member when all are excluded (fail-open)",
  	  strategy: "Outbound strategy",
	  strategyRandom: "Random parallel: spread connections across all nodes, aggregate bandwidth (may slow short-lived connections)",
	  strategyRoundRobin: "Round-robin parallel: alternate connections across all nodes, aggregate bandwidth (may slow short-lived connections)",
	  strategyLeastPing: "Lowest latency: long-lived connections reused, fastest single stream (default)",
  	  agentURL: "Agent URL, e.g. http://1.2.3.4:19528",
    agentToken: "Agent Token",
    controlOk: "Control command sent: {action}",
  },
